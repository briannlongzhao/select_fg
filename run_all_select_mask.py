...
class_list_coco = [name for name in
                   json.load(open("/lab/tmpig8e/u/brian-data/COCO2017/VOC_COCO2017/label2id.json")).keys()]
pick_list = {'aeroplane': 50, 'bicycle': 50, 'bird': 50, 'boat': 50, 'bottle': 50, 'bus': 50, 'car': 50, 'cat': 50,
             'chair': 50, 'cow': 50, 'diningtable': 50, 'dog': 50, 'horse': 50, 'motorbike': 50, 'person': 100,
             'pottedplant': 50, 'sheep': 50, 'sofa': 50, 'train': 50, 'tvmonitor': 50}
step_list = {'aeroplane': 50, 'bicycle': 50, 'bird': 50, 'boat': 50, 'bottle': 50, 'bus': 50, 'car': 50, 'cat': 50,
             'chair': 50, 'cow': 50, 'diningtable': 50, 'dog': 50, 'horse': 50, 'motorbike': 50, 'person': 100,
             'pottedplant': 50, 'sheep': 50, 'sofa': 50, 'train': 50, 'tvmonitor': 50}
...
if len(sys.argv) == 2:
    ...
    class_list_coco = class_list_coco[int((part - 1) * len(class_list_coco) / num_machine):int(
        part * len(class_list_coco) / num_machine)]
    print(class_list_coco)

for class_name in class_list_coco:
    if class_name not in ["toaster", "baseball bat", "backpack", "baseball glove", "bowl", "book", "dining table",
                          "knife", "mouse", "skis", "hair drier", "handbag", "snowboard", "spoon", "sports ball",
                          "truck", "toothbrush"]:
        continue
    try:
    ...
    os.system(
        "python " + option + script + " --img_root " + img_dir + " --mask_root " + mask_dir + " --extraction_root " + extraction_dir + " --save_root " + save_dir + " --num_iter 3 " + " --pick " + pick + " --pick_step " + pick_step + " --method " + method + " --dataset " + dataset + " --target_class \"" + class_name + '\"')
